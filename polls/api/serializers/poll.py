...
class PollSerializer(serializers.ModelSerializer):
    ...
    def validate_options_data(self, value):
        ...
        if len(normalized) != len(set(normalized)):
            raise serializers.ValidationError("All option texts must be unique.")
        return value
    options_data = serializers.ListField(
        child=serializers.CharField(max_length=255, help_text="Text for a poll option (choice)."),
        write_only=True,
        required=True,
        min_length=2,
    ...
    expires_at = serializers.DateTimeField(required=False, allow_null=True, help_text="Optional expiration date/time (ISO 8601). Null means no expiry.")
    is_active = serializers.BooleanField(default=True, required=False, help_text="Is the poll open for voting? Defaults to True.")
    options = OptionSerializer(many=True, read_only=True, help_text="List of poll options (choices). Read-only.")
    created_at = serializers.DateTimeField(read_only=True, help_text="Poll creation timestamp.")
    options = OptionSerializer(many=True, read_only=True)
    
    expires_at = serializers.DateTimeField(required=False, allow_null=True)
    class Meta:
        model = Poll
        fields = ['id', 'question', 'expires_at', 'is_active', 'options', 'options_data', 'created_at']
