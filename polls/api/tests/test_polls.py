...
class PollAPITests(APITestCase):
    ...
    def test_authenticated_vote(self):
        ...
        response = self.client.post(
            url, {'option': self.option1.id}, format='json')
        self.assertIn(response.status_code, [
                      status.HTTP_201_CREATED, status.HTTP_200_OK])
        self.assertEqual(Vote.objects.filter(
            poll=self.poll, user=self.user).count(), 1)
    ...
    def test_authenticated_vote_update(self):
        ...
        # Change vote
        response = self.client.post(
            url, {'option': self.option2.id}, format='json')
        self.assertIn(response.status_code, [
                      status.HTTP_201_CREATED, status.HTTP_200_OK])
    ...
    def test_guest_vote(self):
        ...
        response = self.guest_client.post(
            url, {'option': self.option1.id}, REMOTE_ADDR=ip_address, format='json')
        self.assertIn(response.status_code, [
                      status.HTTP_201_CREATED, status.HTTP_200_OK])
        self.assertEqual(GuestVote.objects.filter(
            poll=self.poll, session_id=session_id, ip_address=ip_address).count(), 1)
